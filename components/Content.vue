        ...
        <div class="card bg-white rounded-lg p-4 ml-16 cursor-pointer">
          <div class="card-header clearfix">
            <div class="shoutby">
              <img
                :src="
                  require(`@/assets/images/profiles/${shout[0].shoutByImg}`)
                "
                width="45"
                height="45"
                :alt="shout[0].shoutBy"
            ...
            <div class="flex items-center">
              <div class="shoutto flex flex-row items-center">
                <img
                  :src="
                    require(`@/assets/images/profiles/${shout[0].shoutToImg}`)
                  "
                  width="45"
                  height="45"
                  :alt="shout[0].shoutTo"
              ...
              <div class="flex flex-col mx-2">
                <div class="flex items-center divide-x divide-gray-200 ">
                  <div class="font-medium text-lg leading-none pr-2">
                    {{ shout[0].shoutBy }}
                    <span v-if="shout[0].moreShouter > 1"
                      >and {{ shout[0].moreShouter }} others</span
                    >
                  </div>
                  <div
                ...
                >
                  <img
                    ...
                    class="mr-1 "
                  />
                  <div class="mr-1">{{ shout[0].shoutDay }},</div>
                  <div class="mr-1">
                    {{ shout[0].shoutMonth }} {{ shout[0].shoutDate }}
                  </div>
                  <div class="dot"></div>
                  <div class="ml-1">{{ shout[0].shoutTime }}</div>
                </div>
              </div>
            </div>
          </div>
          ...
          <div class="card-footer">
            <div class="tags-wrapper flex items-center mb-6">
              <img
                ...
                class="mr-1 "
              />
              <div v-for="(tag, index) in shout[0].tags" :key="index">
                <nuxt-link
                  :to="tag.url"
...
export default {
  ...
  created() {
    this.$nuxt.$on('delectActiveBlockClass', (info) => {
      this.delectActiveBlock(info)
    })
    this.$nuxt.$on('removeCommentBlock', (hideComment) => {
      this.isCommentActive = hideComment
    })
  },
  methods: {
    selectItem(i, id) {
      this.hoverItem = this.shout[1].i
    },
    deselectItem(i, id) {
      const elem = document
        .getElementById(id)
    ...
    delectActiveBlock(ele) {
      document
        .getElementById(ele[0])
        .getElementsByClassName('card-wrapper')[0]
        .classList.remove('active', 'active-block')
    }
  }
}
</script>
...
.card-wrapper.active .toolbox-shadow {
  display: block;
  top: -1.6rem;
  animation-name: showuptoolboxShadow;
